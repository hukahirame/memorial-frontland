...
## [D-018] ファイルの移動は .meta と対で行い、GUID の一致を照合する
status: Accepted
date: 2026-09-10
scope: AGENTS.md, Assets/

**背景** — エージェントによるファイル移動を「GUID が壊れる」という理由で禁止していた。
しかし壊れるのは移動そのものではなく、`.meta` を置き去りにしたときだけである。
GUID は `.meta` の中にあり、対で動かせば保たれる。実測でも 33 組を移動して
51 件すべての GUID が一致した。禁止したまま運用した結果、例外許可が日常的に
発生していた。

**検討した選択肢** —
(a) 禁止を維持する。→ 却下。**日常的に例外が出る禁止は機能していない。**
規則が多いほど守られなくなるという性質は既に観測しており、実態と合わない
禁止を残すと他の規則の信頼性まで下げる。
(b) 無条件に許可する。→ 却下。失敗が静かだから。GUID が壊れた症状は
Missing Script で、そのシーンを開くまで気づかない。**検出できない失敗を
許可してはいけない。**

**決定** — `.meta` を持つファイルは、本体と `.meta` を必ず対で移動する。
`.cs` に限らず prefab・テクスチャ・asmdef・フォルダも同じ。
移動の前後で対象すべての GUID が一致することを照合し、`.meta` の欠落と
孤立が 0 であることを確認する。1 件でも変わっていたら差し戻す。

権限境界の一次基準「Git で差分レビューできるか」に照らすと、純粋な移動は
rename として出て内容の差分が 0 になるため、レビュー可能性が最も高い操作である。
シーンと prefab への書き込みを禁じている理由（検証不能）とは正反対にあたる。

**帰結** — 照合を省いた移動は禁止のままとなる。判断が誤りだった場合の
検出条件: 照合を通ったのにシーンで Missing Script が出ること。なお既存の
シーンには以前からの Missing Mono Script が残っており、目視では新旧を
区別できない。だから機械での照合を必須にしている。

## [D-019] ゲームデータの置き場はテーブルごとに決める
status: Accepted
date: 2026-09-10
scope: docs/conventions.md, Assets/_Project/Resources/, Assets/_Project/Data/

**背景** — レシピは ScriptableObject、アイテム原簿は CSV という混在状態にあり、
どちらを選ぶかの基準が書かれていなかった。ItemCatalog で CSV の解析を Domain に
置いた際、レシピを SO へ移した方針と逆に見えたことで、基準の不在が表面化した。

**検討した選択肢** —
(a) すべて ScriptableObject に統一する。→ 却下。**編集の速さは形式ではなく件数で
決まる。**8 件を Inspector で 1 つずつ触るのは表計算より遅く、行数が増えるほど
不利になる。原簿はまさにその形。
(b) すべて CSV に統一する。→ 却下。入れ子と可変長を表で表せず、固定枠を作って
空欄で埋めることになる。CraftData.csv が実際にその形で、13 列のうち材料スロット
4 つぶんが固定枠、ほとんどが空だった。材料が 5 つのレシピは表現できなかった。
**表で表せない構造を表に入れると、列が増えて中身が空になる。**
(c) スプレッドシートを編集元にし、ビルド時に SO を生成する。→ 今は採らない。
規模に対して変換器の維持費が見合わない。数百行になったら再検討する。

**決定** — テーブルごとに決める。入れ子・可変長・他アセットへの参照があるものは
ScriptableObject にする。平坦で行数が多く一括編集するものは CSV に置き、Domain で
解析してテストする。迷ったら ScriptableObject にする。入力を属性で縛れるぶん壊れにくい。

**帰結** — 混在が続くため、データを足すたびに判断が要る。判断が誤りだった場合の
検出条件: CSV に固定枠の空列が増え始めること。そのときは ScriptableObject へ移す。
