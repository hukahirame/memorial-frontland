...
| `docs/slices.txt` | 型が属する関心事の表。コードの索引 |
...
`docs/slices.txt` のどれかの行に足します。すべての型がちょうど1つのスライスの
核に入っていることを `dotnet test` が見張るので、忘れると落ちます。
表はコードの索引を兼ねています。

依存を図にする仕組み（現状図・差分図・`tools/diagram-diff.ps1`）は廃止しました。
残したのは上の被覆検査だけです。